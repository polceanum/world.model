...
**Current amendment:** general-capability factor evaluation, configurable set capacity, packed active-pair execution, compact static progress reporting, manifest-scoped 250 MiB artifact retention, and per-object worst-slice prediction acceptance, 8--13 September 2026
...
boundary was `1130 passed, 16 skipped in 414.82 s`; the first integrated bridge
boundary passed `1207 passed, 16 skipped in 431.10 s`, and canonical comparator
source in section 250 passes `1209 passed, 16 skipped in 434.37 s`.
...
the accepted lower bound.  At the specification-1.55 source-freeze boundary,
the first public integration existed behind the ordinary `OnlineWorldModel`
contracts and its qualification data was unopened.  Section 250 records the
subsequent completed qualification.  The bridge consumes exactly one
composite, batched `rgbd` packet per
...
At the specification-1.55 source-freeze boundary all four namespaces were
unopened.  No development, selector, confirmation, final, online-bridge
accuracy, or public long-horizon-convergence result existed at that boundary.
The bridge source, configuration, and qualification protocol had passed
independent source review; that was pre-development contract evidence, not
episode evidence.
...
The final canonical-comparator targeted bridge gate is
`421 passed in 62.72 s`; the complete repository gate is
`1209 passed, 16 skipped in 434.37 s`.  The module has zero
...
The source-freeze allowed sequence was: commit and push the clean reviewed
source; run development once to fresh artifacts; independently audit its
...
ledger.  Section 250 records that completed sequence.  Camera motion, variable
parameters, additional objects, contact, learned capacity, and new modalities
remained closed until this bridge reproduced the standalone accuracy,
gradient, failure-atomicity, memory, and latency gates.

## 250. Accept the exactly-once public bridge qualification

The first clean development on source `ebda5a8` had all `175` reported scalar
metrics finite and passed all frozen gate checks, but its post-run audit found
a representation-sensitive comparison
between raw Python tuple/list protocol values after JSON roundtrip.  The
numerical result is therefore rejected promotion evidence even though no gate
failed.  No protected namespace was accessed and no protected ledger was
created.  The rejected bytes are preserved under ignored directory
`runs/rgbd_online_bridge_v1/rejected_ebda5a8_json_protocol/`; report and
...
protocol JSON and rejects protocol tampering.  Its focused current-byte gate
is `421 passed in 62.72 s`; its complete repository gate is
`1209 passed, 16 skipped in 434.37 s`.  The qualification audit binds runtime
fingerprint
...
to the unchanged config and protocol SHA-256 values from section 248.

The fresh canonical 24-seed development pass over
`45000000--45000023` has report SHA-256
`dce6f920da85fbf696b7ae8a7a91d9cbf7d9084176e51ad7c319f92a6efe4966`
(`22,346` bytes), checkpoint SHA-256
`48249f1a5a0467b1da8c7bdb5ad9e909f8c502631ec2fbad832cb490a00c3099`
(`46,596` bytes), and manifest SHA-256
`069eb3331543727c911a07cc9a1bb352f6185ac8ceac7fafca502c9d7fab6d80`.
All `175` reported scalars are finite and all frozen comparisons pass.
Current position/velocity RMSE is `3.068470 mm`/`2.191966 mm/s`;
two-second position/velocity RMSE is `5.609913 mm`/`1.983371 mm/s`;
position-error growth slope is `1.270721 mm/s`; perception is
`0.415134 s`; five-query state rollout is `3.575380 ms`; and complete
batch-four persistent runtime tensor state is `25,364` bytes.  Every required
target/modality history VJP reaches `16/16` frames, with minimum per-frame L1
`2.9239957802928984e-4`.  Learned parameters, module buffers/state-dict
entries, optimizer and optimizer-scheduler state, and updates are all zero.

Protected qualification then passed selector, confirmation, and final exactly
once under the ledger.  Qualification-report SHA-256 is
`7fd1829f663606910ac81990e4b633c63b1460dbc31dd24c71eedbd91b422908`
(`47,353` bytes); ledger SHA-256 is
`cf6a10dd672aafbdd91c92871ae349fef0c549d865cc6532e6c42f7d9be14e32`
(`1,626` bytes).  Initial and final empty state share SHA-256
`4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945`.
The exact protected bindings are:
...
| split | current position / velocity | two-second position / velocity | growth slope | perception / rollout |
| --- | --- | --- | --- | --- |
...
Every split has `175/175` reported scalar metrics finite with no frozen gate
failures; every required history VJP
reaches `16/16` frames, active fraction is one, and identity changes are zero.
The RGB-only, missing-depth, no-foreground, semigroup, memory, checkpoint, and
zero-state ablations/gates pass.  Final persistent runtime state is `25,364`
bytes, maximum RSS is `708,853,760` bytes, and maximum semigroup disagreement
is `2.384186e-7 m`/`1.862645e-9 m/s`.

The ledger is complete and stopped after final.  Final is consumed and must
never be rerun or tuned against.  Evidence is hash-bound and atomically
replaced, but the ledger and reports remain owner-writable rather than
append-only OS-enforced storage; this is a disclosed operational limitation.
The audit did not reinspect or rematerialize raw protected episodes.

At the specification-1.55 boundary, the one-sphere, fixed-camera, contact-free
public RGB-D bridge was accepted and ready to merge to `main`; section 251
records the completed merge.  This does not establish
multi-object association, occlusion, contact, task/planning, added modality,
learned-capacity, or broader world-model convergence.  The next bounded rung
must predeclare those changed capabilities and their per-object,
association, event/contact, task-success, gradient, accuracy, memory, and
rollout-throughput gates before generation.  It may not alter the accepted
bridge or tune any threshold, architecture, or capacity on the consumed final.

# Part XLVII — Two-visible-object RGB-D source-freeze amendment

## 251. Freeze architecture attempt 2 before any episode access

Specification 1.56 retains the accepted one-object bridge, merged to `main` at
`3eed0b71e6f18c7036bf376c075493a89d5fdc9f`, unchanged, and adds one bounded
capability only: exactly two fixed-radius spheres in a fixed
camera, fixed-parameter, zero-gravity free-motion scene.  Every sphere must be
fully visible in every rendered frame, the two image silhouettes must remain
separated, and their world surfaces must remain non-contacting.  The scene
constructor preflights those properties frame by frame; generic
`ensure_collision: false` is not evidence.  Occlusion, reappearance, variable
set size, contact, camera motion, variable physical parameters, added
modalities, tasks, and learned capacity remain outside this rung.

Architecture attempt 2 is parameter-free.  A differentiable symmetric
chromatic-plus-spatial two-slot RGB-D geometry module produces unordered
metric measurements for both objects.  RGB chromaticity and spatial evidence
define the two slots continuously; observable depth, known radius, and camera
calibration own metric geometry.  Hard Hungarian assignment is permitted only
as the discrete, stable identity-control branch.  It is not presented as a
differentiable learned component and may not own metric state or gradients.
There is exactly one direct metric-position owner.  Persistent-ID-aligned raw
positions from frames `0--15` enter a uniform differentiable exact free-motion
weighted least-squares fit that owns velocity, and parameter-free analytic
dynamics own position and velocity rollouts at
`0.10/0.25/0.50/1.00/2.00 s`.  The accepted one-object path remains supported
without semantic change.

The frozen byte and canonical-payload bindings are:

- config `configs/rgbd_two_visible_free_motion_cpu.yaml`, SHA-256
  `84e6f44b818bb9323a774bdba9492ef056e2a2747b93517fa38497ba83218bba`;
- qualification harness
  `world_model/training/rgbd_two_visible_free_motion_qualification.py`,
  SHA-256
  `198cac1c4d683e3c983f70c0106827aaf883636d4bd6454e94011c3975c1b64a`;
- harness tests
  `tests/unit/test_rgbd_two_visible_free_motion_qualification.py`, SHA-256
  `d5dd3c18515589b4589e0179a68e29112d45987a513308df022cece5bf75e896`;
- thin runner `scripts/run_rgbd_two_visible_qualification.py`, SHA-256
  `a8e6d9f51380eede3b6a94f085e9741f67883e2740c6203c16aec4a5dcfa1bc1`;
  and
- canonical `bridge_protocol()` payload SHA-256, computed before inserting
  its self-reporting field,
  `42b9dca23fed303d5cee4641c8d8753977a872fc90d0b1086658d7f12b823ea0`.

No clean execution commit, runtime-source fingerprint, or full-worktree
fingerprint is claimed at this uncommitted documentation boundary.  Those
values must be captured from the eventual clean committed tree; documentation
may therefore change a later worktree fingerprint without changing any of the
four frozen config/harness/test/runner file hashes above.

Any change to those frozen bytes or canonical protocol is a new experiment
and cannot reuse the namespaces below.  Their seed lists and canonical
...
| one-shot final | `52000000--52000047` | `b7e8913e938e2f7ae7f937979a60279916ff1a06f071427bcce9f08b0e354e75` |

At this source-freeze boundary all four namespaces are unopened.  No episode,
manifest artifact, report, checkpoint, development ledger, protected ledger,
or accuracy result exists for this rung.  The manifest hashes bind the
predeclared seed lists; they do not imply that the corresponding episodes have
been generated or inspected.

## 252. Differentiability, evidence controls, and permitted execution

Fixed-output vector--Jacobian products are audited independently for each
object, output, modality, and scene.  Current position is intentionally owned
only by the anchor measurement at frame 15: its required reach is `1/16`, and
every non-anchor current-position gradient is exactly zero.  Current velocity
and every declared rollout position and velocity must reach all `16/16` RGB
and depth frames.  The B4 audit uses four distinct scenes per split, takes
floors as minima and ceilings as maxima across those scenes, and requires
exactly zero cross-scene gradient coupling.  This contract separates the
continuous trainable path from hard identity control instead of claiming a
gradient through Hungarian assignment.

Seed-free validation of the exact current files passes:

- the focused qualification harness, `43 passed`;
- the combined accepted one-object/configuration/two-object harness,
  `281 passed in 15.61 s`;
- the complete repository, `1275 passed, 16 skipped in 447.29 s`;
- Ruff lint, Ruff format-check, and diff integrity; and
- two independent audits.

These are source and differentiability checks only, not episode accuracy or
qualification evidence.  The simulator protocol remains `sphere_world_v7`.

The seed-free protocol-inspection command is:
...
Do not execute either command below until the frozen harness, configuration,
tests, runner, specification, and documentation are present in one clean
committed source tree.  Development then has exactly one fixed attempt and
must use fresh report/checkpoint paths:
...
Only a passing development result plus independent exact-digest audit may
authorize the protected command, with the reviewed SHA-256 values supplied
explicitly:
...
The fixed attempt-scoped development ledger is
`runs/rgbd_two_visible_bridge_v1/development_attempt_2_access.json`; the
exactly-once protected ledger is
`runs/rgbd_two_visible_bridge_v1/qualification_attempt_2_access.json`.  Both
are durable fresh-path artifacts.  Each records access before materialization;
only a ledger may issue the single-use manifest authorization required by the
scene constructor.  Protected order is exactly selector -> confirmation ->
final.  Each predecessor must pass before the next authorization is created.
The restricted checkpoint is empty-state, optimizer-free, RNG-free, and
loaded with `weights_only=True`.  On terminal success or failure, the report
is written before the ledger receives its terminal report digest.  A failed
development or protected split stops the attempt, leaves later namespaces
unopened, and permits no retuning, renamed retry, or final reuse.

## 253. Interpretation and next boundary

This freeze demonstrates that the structured inductive bias has an ordinary
differentiable RGB-D-to-metric-state-to-analytic-rollout path while discrete
identity bookkeeping remains isolated.  Because this architecture owns zero
learned parameters and performs zero optimizer updates, it is a convergence
baseline and trainability proof, not a learned-capacity result.  Learned
residual capacity may be added later only for a measured residual that this
baseline cannot represent, with its own gradient-owner, non-regression,
efficiency, and disjoint-manifest contract.

The current rung does not establish partial-visibility association, identity
recovery after a miss, occlusion, contact/event prediction, uncertainty
calibration, variable scale or drag, moving-camera handling, actions,
planning, richer modalities, or general world-model convergence.  Ordinary
project checkpoints bind the zero-state module/configuration but do not
serialize live temporal histories, so exact mid-history resume remains
unsupported.  At the source-freeze boundary, no claim could advance beyond the
frozen exactly-two-visible, image-separated, non-contact family until
development and the ordered protected sequence passed exactly as declared.
Section 254 records that subsequent accepted sequence without rewriting the
historical pre-access contract above.

## 254. Accept the exactly-once two-visible-object qualification

The exact source/config/protocol freeze from sections 251--253 was committed
and pushed at commit and origin
`3b781e653a0287b2aa926e7c0b969e9197d48e42`.  Qualification binds runtime-source
fingerprint
...
development-ledger SHA-256 values are respectively
...
The subsequently completed qualification-report and protected-ledger
SHA-256 values are
...
The exact manifest/result bindings are:
...
Every split reports all `396` gated scalar metrics finite and
`gate_failures: []`.  The principal held-out errors are:

| split | current position RMSE | current velocity RMSE | 2 s position RMSE |
| --- | --- | --- | --- |
...
Final two-second velocity RMSE is `2.8845629e-5 m/s`.  The worst final
per-object-axis position and velocity RMSE values are
`1.020557e-4 m` and `4.503076e-5 m/s`.  Identity coverage is one; identity
switches, persistent-ID mismatches, and association ambiguities are zero.  The
minimum Hungarian assignment margin is approximately `199.96`, all objects
remain fully visible, preflight event count is zero, minimum silhouette gap is
`9.534 px`, and minimum world-surface gap is `0.75985 m`.

Every split preserves the declared VJP topology: current position reaches only
one anchor frame with exact zero non-anchor gradient; current velocity and
every rollout position/velocity reach all `16/16` RGB and depth frames; all
four B4 audit scenes are distinct; and cross-scene coupling is exactly zero.
On final, the minimum total VJP L1 is `2.0798e-5`, minimum temporal per-frame
L1 is `5.7799e-8`, and maximum VJP L1 is `4.2197`.

Final perception latency is `0.352510 s`; all five state-only rollout queries
take `0.00359524 s`.  Persistent runtime tensor state is `28,512` bytes and
maximum RSS is `579,817,472` bytes.  Learned parameters, module buffers,
model-state entries, optimizer/scheduler state, RNG state, and optimizer
updates remain zero.  The focused, combined accepted one-object/configuration/
two-object, and complete source gates are respectively `43 passed`,
`281 passed in 15.61 s`, and
`1275 passed, 16 skipped in 447.29 s`; the exact independent final audit also
passes.

The development ledger is complete and passed.  The protected ledger is
complete in exact selector -> confirmation -> final order, and final is
consumed.  It must never be rerun, tuned against, or repurposed.  The audited
run directory contains exactly five single-link regular files, with no
temporary file, alias, or second-attempt artifact.  Audit recomputed persisted
evidence without reopening raw protected episodes.  The files remain
owner-writable, so SHA/fresh-path binding makes them tamper-evident rather than
OS-enforced write-once storage.

This accepts only the exactly-two, fully-visible, image-separated,
