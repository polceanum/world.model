...
**Amendment 24 August 2026:** RGB-only photometric sphere geometry with differentiable learned-head carriers
...
## 269. Train through a soft posterior while deploying the hard posterior

When `training.closed_loop_soft_posterior_straight_through_enabled` is true,
training evaluates a second analytic diagonal correction under the same gated
soft assignment defined in section 268. The soft correction is attached to
position, supported direct velocity, and their log variances only through a
zero-valued straight-through carrier. The persistent forward value is therefore
bit-exact to the complete hard posterior after Hungarian association,
lifecycle, learned correction, direct velocity, and physical identification,
while backward gradients from later observations and equation-based rollouts
can cross an assignment boundary.

The carrier may not introduce a learned transition, replace hard identity,
birth or delete an object, or alter any deployed tensor. It uses the same
measurement confidence, independent-axis provenance, robust analytic Kalman
update, variance bounds, and admissibility gate as the runtime filter. It is
installed into `RuntimeState` only during gradient-enabled training so the next
causal frame and posterior rollout retain its graph; ordinary ingest,
validation, evaluation, checkpoints, and deployment never request it.

The switch is strict boolean, legacy-false, and exact-resume-bound. Enabling it
...
## 270. Train continuous belief geometry by RGB-only analytic reprojection

The supported differentiable state-estimator phase may add an exact-weight
`rgb_reprojection` objective. It constructs a soft foreground silhouette from
the active spheres in the current `WorldBelief` using calibrated perspective
projection, their stored radii, and a one-pixel smooth edge. The target is
derived only from the current RGB frame by robust per-row background removal;
simulator segmentation, object identity, target state, and future labels are
forbidden. The objective is training-only and may not mutate `WorldBelief`,
association, lifecycle, caches, runtime outputs, or deployment latency.

Multiple projected spheres combine as a differentiable silhouette union. The
loss balances foreground and background pixels independently within each batch
row so empty sky cannot dominate. Unsupported rows contribute exact zero and
report explicit support. This provides dense gradients to projected x/y,
depth through apparent scale and perspective, and sphere radius while leaving
Hungarian identity, births/deaths, hard visibility order, contacts, and the
analytic dynamics equations unchanged. It adds neither a learned transition
nor a second rollout.

The objective is opt-in, exact-resume-bound with legacy value zero, and requires
the RGB `differentiable_state_estimator` scope. On the first exact balanced
training draw, weight `2.0` uses 32 supported frame rows, 96 projectable
objects, and 2,237 observed foreground pixels. Its raw gradient norm is
`0.279300`; the weighted norm is `0.558601`, or `9.54%` of the combined
`5.854154` norm, with cosine `-0.0471` against the remaining objective. This
training-draw attribution freezes the sole treatment weight before accuracy
evaluation.

One 16-update paired treatment/control qualification must differ only in
`loss_weights.rgb_reprojection` (`2.0` versus `0.0`) and must pair every draw,
scenario, initializer tensor, and initial hard physical metric. Treatment must
improve the equal-weight five-horizon position selector by at least `0.001`;
current position/velocity and every configured position/velocity horizon may
worsen by at most `0.5%`; coverage, precision, identity, collision F1, and
calibration retain the established non-regression tolerances. Failure closes
...
## 271. Train through a true smooth posterior and the existing physical equations

A training-only `soft_shadow_physical` objective may replace supported
continuous position, velocity, and diagonal uncertainty rows in a detached
copy of the hard posterior with the true analytic Kalman result under the
gated soft association distribution. Integer identity, lifecycle, active
masks, unsupported objects, and every other typed field remain detached copies
of the hard runtime result. The shadow is never installed into `RuntimeState`.

The smooth posterior must be propagated by the existing `DynamicsModel`
rollout, including the same analytic kinematics, drag, gravity, boundaries,
contacts, event construction, and uncertainty evolution used by deployment.
It may not introduce a learned transition or simulator state as runtime input.
Ground-truth state remains permitted only as the fixed supervised target, as
in the ordinary closed-loop physical objective. Hard target alignment is a
label selection operation and is not claimed to be differentiable.

The shadow composite mirrors the resolved current position/velocity,
axis-specific or aggregate rollout-position, rollout-velocity, and rollout-NLL
weights. Its sole outer scale is opt-in, exact-resume-bound, and legacy zero.
A positive value requires the RGB differentiable-state-estimator scope and a
positive soft-association temperature. Zero must not build the shadow or its
additional rollout. Validation and deployment remain the unmodified hard path.

Before any long campaign or merge to `main`, the implementation must pass
hard-runtime isolation, finite-gradient, configured-weight, exact-resume, and
complete repository tests, then one frozen paired short accuracy gate against
exact weight zero. The pair must predeclare materiality and non-regression
...
## 274. Treat renderer geometry as a qualified RGB measurement, not a runtime oracle

The RGB observer may opt into a detached inverse-rendering fit for complete,
unambiguous sphere silhouettes. The fit uses only RGB pixels, the renderer's
radial Lambertian image profile, and the structured image-derived centre. It
jointly estimates subpixel centre, apparent radius, and albedo, rejects border,
partial, merged, weak-edge, and high-residual components, and exposes no
simulator state. Accepted radius and inverse depth retain their exact analytic
forward values while straight-through carriers provide gradients to the learned
fast-ROI radius and depth heads. Analytic backprojection, Kalman correction,
temporal estimation, and equation-based dynamics remain unchanged.

`model.rgb.structured_disc_photometric_fast_depth_enabled` is a strict boolean,
legacy-false, and exact-resume-bound. It requires structured centres and the
existing structured fast-depth path. The maximum RGB fit RMS is finite,
positive, configurable, and exact-resume-bound. The generic and supported
profiles remain disabled. A CPU implementation is acceptable only as research
instrumentation; deployment additionally requires measured active-device
latency and must not depend on the detached fitter when a learned student is
expected to replace it.

The image-model fit itself is accurate on the held-out renderer diagnostic:
460 qualified samples recover radius with `0.333%` mean absolute relative error,
`0.598%` p95 error, and `0.014` pixel mean centre error. An oracle-anchor
decomposition separately reduces fixed-eight 0.1-second position error from
`0.140636` to `0.005056 m`, proving that state observation rather than the
analytic rollout is the dominant short-horizon bottleneck.

Direct runtime substitution is nevertheless rejected. On the complete fixed-32
manifest it improves the pooled selector `0.239528636 -> 0.237682466`, current
velocity `0.811882 -> 0.748072 m/s`, pooled collision F1
`0.283747 -> 0.310811`, and most position horizons, but current position improves
only `0.150565 -> 0.149861 m` and 291 broad scenario/axis/horizon guardrails
fail. Failures are dominated by Gaussian NLL, local velocity, calibration, and
scenario-specific position; 1-second event and velocity behavior also regress.
Switching component selection from the prior centre to the structured centre
improves fixed-eight position horizons and short velocity but still worsens
current position and 1-second collision/velocity, closing further threshold or
fit tuning.

The mechanism is therefore retained default-off as differentiable research
infrastructure, not promoted and not merged to `main`. Any successor must use
the fit as training-only teacher evidence or an uncertainty-aware learned fusion
input, deploy the learned RGB student without the CPU optimizer, and pass one
predeclared paired fixed-manifest accuracy and latency gate. Failure of that
bounded successor closes this line; repeated threshold, residual, or duration
sweeps are not permitted.

The final repository gate passes `1383` tests with `20` expected
unavailable-backend skips in `536.52 s`; Ruff, format, version, and diff checks
are clean. The fixed-32 report is
...
## 275. Derive depth from a learned image quantity and a differentiable camera equation

The fast RGB observer may opt into a calibrated sphere-projection path in which
the ROI network predicts apparent log radius and the measurement module derives
inverse depth from focal length, image size, and the configured physical sphere
radius. The analytic camera equation remains inside the ordinary PyTorch graph:
the depth objective backpropagates into the learned radius row, while no separate
unconstrained inverse-depth residual is trained. This is the preferred pattern
for differentiable physical inductive bias: learn the image quantity that is
observable, then transform it with the known equation rather than asking a
black-box head to relearn both.

`model.rgb.fast_radius_derived_depth_enabled` is a strict boolean, defaults
false, has a legacy-false exact-resume migration, and is mutually exclusive with
both unconstrained fast depth residuals and structured-disc depth substitution.
The disabled path is unchanged. Focused coverage proves the exact projection,
nonzero radius-row gradient, zero direct depth-row gradient, strict configuration,
and checkpoint semantic binding. Separately, normal `OnlineWorldModel` creation
must pass `runtime.hypothesis_shared_horizon_rollout_enabled` into the runtime
hypothesis controller; evaluator-only wiring is insufficient. A construction
regression protects that integration path.

The bounded evidence rejects promotion. A zero-update fixed-eight screen is
mixed: position improves at 0.1--0.75 seconds by `5.82e-6` to `2.16e-5 m`, but
current and 1-second position regress slightly. One frozen 16-update balanced
CPU treatment/control pair then demonstrates real optimization value without
accuracy value. The derived-depth treatment keeps maximum raw perception
gradient at `0.922627` with zero local clips, while the exact control reaches
`8579.301758` and clips once. Nevertheless treatment versus control worsens
current position by `0.000229989 m`, worsens current velocity by
`0.000008844 m/s`, regresses four of five velocity horizons, and improves only
collision F1 and 1-second position slightly. Both arms also regress from the
common initializer on current position, velocity, and collision F1.

This candidate therefore stops before fixed-32, MPS, longer training, or any
learning-rate/weight/duration sweep. The mechanism remains default-off as a
sound differentiable equation path and gradient-stability diagnostic, but is
not eligible for `main`. The dominant accuracy conclusion remains the oracle
anchor result: equation rollouts are already close to the simulator when the
initial world state is correct, so future work must improve RGB observation and
association evidence without sacrificing the fixed physical guardrails.

Evidence is retained at
`/private/tmp/20260824-spec176-learned-radius-derived-depth-fixed8.json`
(`e7fe61f2...`) and the paired run roots
`/private/tmp/20260824-210000-spec176-radius-depth-treatment16` and
`/private/tmp/20260824-211500-spec176-radius-depth-control16`. Their resolved
config SHA-256 values are `52d47503...` and `547d57ad...`; metrics SHA-256
values are `d1fdcb21...` and `41169998...`; validation-step-16 checkpoint
SHA-256 values are `b8b53dde...` and `8cabddc1...`.

The final repository gate on these source bytes passes `1386` tests with `20`
expected unavailable-backend skips in `520.50 s`.

---

## 276. Preserve continuous observation ambiguity through differentiable equation rollouts

Hard identity, lifecycle, and contact decisions remain typed runtime structure;
they are not trainable scalar approximations. Continuous state ambiguity must
not, however, be collapsed by a hard prior-versus-observation choice before
physical supervision. A training-only `HypothesisSet` may therefore retain a
small number of continuous `WorldBelief` modes that share exact object IDs and
active masks. Every mode is propagated by the same configured dynamics
equations, and current/future targets are scored by a proper Gaussian-mixture
log likelihood using `logsumexp`. No target-conditioned `argmax`, straight-
through hard selector, or simulator state enters runtime inference.

The first bounded form contains exactly two modes: the propagated equation
prior and the deployed RGB-corrected posterior. It reuses the detached prior
trajectory already required by the correction-improvement objective and the
ordinary differentiable posterior trajectory, so enabling the objective must
not add a dynamics rollout. The feature is controlled only by the exact-resume-
bound `training.loss_weights.observation_hypothesis_nll`, whose legacy and
default value is zero. A positive value requires RGB runtime, the
`differentiable_state_estimator` scope, and the shared prior-future rollout.

This mechanism is research-qualified only until accuracy evidence exists. The
real balanced attribution at candidate weight `0.1` reports a `9.723%`
hypothesis-to-combined gradient ratio, finite support on 94 current objects and
235 future object-horizons, exact pre-existing losses/physical metrics, and no
measurable forward overhead (`15.1887` versus `15.2041 s`). The new gradient
has cosine `-0.37794` with the existing objective, so one predeclared paired
short accuracy gate must decide whether the anti-collapse pressure is useful.
No weight/duration sweep is permitted. Promotion or merge to `main` requires
material fixed-manifest improvement with all existing scenario/axis/horizon,
...
## 277. Bind runtime-hypothesis evidence to independent RGB world axes

Online hypothesis selection may learn only from coordinates that the incoming
observation independently measured. A source-conditioned FAST ROI packet often
copies camera depth from the predicted belief while directly observing only
image-plane centre coordinates. Treating all three reconstructed world
coordinates as fresh evidence lets a candidate score itself against its own
prior and is forbidden.

`HypothesisRolloutEngine`, `HypothesisDynamicsPool`, and
`RuntimeHypothesisController` therefore carry an explicit boolean
`target_position_axis_mask`. Position losses, evidence counts, observability,
local residual correction, and predictive-variance aggregation use only those
axes. An explicit axis mask must be a subset of the scalar target mask. A
source-bound RGB measurement without axis provenance fails closed and causes
the controller to abstain unless independent velocity evidence exists. A
global/unbound RGB observation retains the historical all-axis interpretation,
and omitting the new argument is tensor-exact with an explicit all-axis mask.

This is a correctness and evidence-integrity repair, not a runtime-pool
promotion. The bounded fixed-eight RGB/CPU pair used the same source,
checkpoint, manifest, and policy. The candidate exercised 68 supported x-axis
cells, 42 learned selections, 26 constant-velocity selections, and 592 fallback
query cells, so the intervention was non-vacuous. Its sum of pooled position
improvements across 0.10/0.25/0.50/0.75/1.00 seconds was only
`0.0000024393 m`, below the mandatory `0.00001 m` materiality floor. Measured
global, fast, and rollout latency ratios were `1.2513x`, `1.2360x`, and
`1.6683x`, all above the `1.10x` ceiling. Collision F1, identity, and current
posterior metrics were unchanged.

The runtime pool therefore remains disabled and is not eligible for `main`.
No threshold, weight, duration, or fixed-32 continuation is authorized. The
axis-evidence repair remains on the research branch because it prevents
invalid self-evidence in any future pool implementation. Reference and
candidate reports are respectively
`/private/tmp/20260825-003457-20260825-spec180-axis-aware-pool-reference-fixed8/evaluation.json`
(`41e97466...`) and
...
## 280. Train free motion through the deployed equations without recursive-grid backpropagation

The hybrid runtime is not globally differentiable: identity assignment,
lifecycle changes, and contact resolution are typed discrete transitions.
Its continuous observation, Kalman/corrector, uncertainty, analytic
kinematics, and learned residual paths are differentiable, but causal training
previously retained the complete 120 Hz hybrid rollout graph even when the
target interval contained only free motion. On a real balanced updater-head
batch the configured single earliest rollout anchor had exactly zero gradient
to its six trainable owners, despite paying for two recursive forecasts.

`training.closed_loop_free_motion_equation_surrogate_enabled` is therefore a
strict default/legacy-false, exact-resume-bound research switch. In gradient-
enabled training with no event owner, supported objects are propagated
directly from the corrected posterior by the deployed closed-form constant-
gravity/linear-drag integrator. Position uncertainty uses the matching
diagonal equation `Var[p(t)] = Var[p] + t^2 Var[v]`. Training labels only
censor object intervals containing contact, collision, external actuation,
creation, or removal; labels never enter runtime inference. Validation,
deployment, event-owned training, disabled execution, and caller-supplied
trajectories retain the exact hybrid rollout.

The bounded real-batch gate uses four cheap posterior equation anchors and
disables the detached prior-future diagnostic rollout. It executes zero
recursive forecast calls, reduces forward-plus-backward from `14.5724` to
`8.7820 s` (`1.659x`), reduces peak RSS from `961,093,632` to `663,441,408`
bytes, and exposes a finite rollout gradient norm `0.00897988`, about `13%` of
the `0.0688126` current-state gradient. Exact six-head total gradient remains
finite (`0.0723071`). The recursive and equation attribution reports are
`/private/tmp/orpheus_free_motion_recursive_attribution.json` (`4d43b5bf...`)
and `/private/tmp/orpheus_free_motion_four_anchor.json` (`b5b2432e...`).

One two-update end-to-end CPU smoke then completed from the immutable 1.52
initializer with the exact eight-scenario draw order. Both updates were finite
and unclipped; exactly the mean, variance, and gate head weights/biases changed
and exactly those six tensors acquired Adam state. Exact hybrid validation
before/after is unchanged at warm-up scale (roughly `1e-7`), so this is wiring,
ownership, and cost evidence—not accuracy or promotion evidence. The retained
run is `/private/tmp/20260825-031744-20260825-spec181-free-motion-two-update`;
its final checkpoint SHA-256 is `80395287...`.

Final repository verification passes `1417` tests with `20` expected
...
## 281. Bind the exact recursive rollout to a causally owned posterior

Bounding causal training to one rollout anchor historically selected the
earliest eligible frame because it exposes the widest horizon set. In a TBPTT
window that frame can precede every correction owned by the active trainable
scope. A real balanced updater-head attribution confirms the result: the
posterior recursive rollout is present and expensive, but its gradient to all
six selected heads is exactly zero.

`training.single_rollout_anchor_policy` is an exact-resume-bound enum with
legacy/default value `earliest`. The opt-in `latest_full_horizon` policy first
computes each eligible frame's configured-horizon support, retains the maximum
support count, then chooses the latest tied frame. It therefore cannot discard
a horizon available to the historical earliest anchor, but it moves the exact
recursive objective after causal corrections made inside the window.
Multi-anchor selection, validation anchors, deployed inference, event/contact
resolution, `WorldBelief`, observations, and rollout equations are unchanged.

On the exact first balanced batch and source-frozen initializer, changing only
the anchor policy gives posterior rollout gradient norm `0.04474605` rather
than `0`, with finite gradients in every mean/variance/gate owner. Compute is
