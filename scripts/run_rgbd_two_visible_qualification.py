#!/usr/bin/env python3
"""Thin CLI for the frozen two-visible-sphere RGB-D qualification."""
...
DEFAULT_RUN = REPOSITORY_ROOT / "runs" / "rgbd_two_visible_bridge_v1"


def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Inspect the two-visible RGB-D protocol, run development only, or "
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/rgbd_two_visible_free_motion_cpu.yaml"),
    )
...
def main() -> int:
    ...
    if args.phase == "development":
        ...
        print(
            "PASSED: two-visible development is review-ready; protected data remains unopened"
            if result == 0
            else "FAILED: two-visible development gates; protected data remains unopened"
        )
    ...
    print(
        "PASSED: selector, confirmation, and one-shot final two-visible gates"
        if result == 0
