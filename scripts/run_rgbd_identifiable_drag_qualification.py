#!/usr/bin/env python3
"""Thin CLI for the frozen identifiable-drag RGB-D qualification."""
...
from world_model.training.rgbd_identifiable_drag_qualification import (
    bridge_protocol,
...
CONFIG_PATH = (
    REPOSITORY_ROOT / "configs" / "rgbd_two_visible_orbital_camera_identifiable_drag_v1.yaml"
)
...
def arguments(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Inspect the identifiable-drag protocol, consume development once, "
            "or consume its externally reviewed protected ladder once."
        )
    )
    parser.add_argument(
        "--phase",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.phase == "development":
        ...
        print(
            "PASSED: identifiable-drag development is review-ready; protected data remains unopened"
            if result == 0
            else "FAILED: identifiable-drag development gates; protected data remains unopened"
        )
    ...
    print(
        "PASSED: selector, confirmation, and one-shot final identifiable-drag gates"
        if result == 0
