...
- A decoder parameter-row cap runs too late to protect shared attention state.
  ...
  output invocation before shared backpropagation, retain later semantic row
  caps for accumulated decoder updates, and report both diagnostic
  hierarchies.
- Treat less than 10% retention at the complete interaction stage after all
