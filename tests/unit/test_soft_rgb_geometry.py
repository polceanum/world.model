...
def _shaded_renderer_profile(
    *,
    ...
    image[:, bands, :] *= 0.82
    radial_squared = ((pixel_x - centre_x).square() + (pixel_y - centre_y).square()) / radius**2
...
def test_soft_rgb_geometry_gradient_matches_central_finite_difference() -> None:
    ...
    def objective(mask_centre_x: torch.Tensor) -> torch.Tensor:
        logits = 2.0 - ((pixel_x - mask_centre_x).square() + (pixel_y - 20.0).square()) / (
            2.0 * 7.0**2
        )
...
def test_gauss_newton_geometry_recovers_100_public_renderer_profiles() -> None:
    ...
    expected_centres = image.new_tensor(tuple(centre for _, _, _, centre, _, _ in profiles))
    expected_radii = image.new_tensor(tuple(radius for _, _, _, _, radius, _ in profiles))
    ...
    assert PHOTOMETRIC_DAMPING_FORMULA == ("sqrt(dtype_epsilon)*mean(diag(J^T_J))+dtype_epsilon")
