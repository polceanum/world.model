...
def _heatmap(summary: CapabilityRunSummary) -> str:
    cells = summary.cell_metrics
    if not cells:
        return '<div class="empty">Physical cell heatmap: unmeasured</div>'
    entries = list(cells.items())
    values = [
        value
        for _, metrics in entries
        if (value := _cell_value(metrics, "current_position_rmse_m")) is not None
    ]
    maximum = max(values, default=1.0)
    blocks: list[str] = []
    for index, (label, metrics) in enumerate(entries):
        value = _cell_value(metrics, "current_position_rmse_m")
        column = index % 6
        row = index // 6
        ...
        if value is None:
            ...
            ratio = min(1.0, value / max(maximum, 1.0e-12))
    ...
    return (
        ...
        'aria-label="count contact lifecycle current-position error heatmap">'
        '<text x="18" y="20" class="chart-title">Current-position RMSE by physical cell</text>'
        f"{''.join(blocks)}</svg>"
    )
...
def _planning_table(summary: CapabilityRunSummary) -> str:
    ...
    return (
        "<table><thead><tr><th>Count</th><th>Candidates</th><th>Winner accuracy</th>"
...
def _summary_section(summary: CapabilityRunSummary) -> str:
    ...
    <section><h2>Physical behavior</h2>{_heatmap(summary)}</section>
...
def render_summary_html(
    ...
    *,
    ...
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">{refresh}<title>{_escape(title)}</title><style>{_STYLE}</style></head><body><main>{_summary_section(summary)}{notice_html}<section class="run-list"><h2>Capability trend</h2>{_svg_line_chart([(item.run_id, _score_value(item, "candidate")) for item in history if _score_value(item, "candidate") is not None], title="Lower-is-better score across runs")}<ol>{history_rows}</ol></section><footer>Portable report · schema {CAPABILITY_SUMMARY_SCHEMA} · no external assets</footer></main><script type="application/json" id="capability-run-summary">{embedded}</script></body></html>"""
...
def build_progress_dashboard(
    ...
    *,
    ...
    latest = CapabilityRunSummary.from_dict(
        {
            **latest.to_dict(),
            "artifacts": {**latest.artifacts, "archive_bytes": archive_bytes},
        }
    )
