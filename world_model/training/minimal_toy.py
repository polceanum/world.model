...
REJECTED_V1_REPORT_SHA256 = "a9cafaad5bd7fcabaebfdd815a89b6fe125284ca270a00b1a782def3bf683ce1"
...
class DifferentiableToyStateEstimator(nn.Module):
    ...
    def forward(
        ...
        inverse_depth = radius_pixels / (focal_pixels[:, None] * self.world_radius_m).clamp_min(
            1.0e-8
        )
...
def _assert_minimal_contract(config: OrpheusConfig) -> None:
    ...
    for name, expected in {**required, **fixed_ranges}.items():
        ...
        if actual != expected:
            raise ValueError(
                f"minimal convergence config requires simulator.{name}={expected!r}, got {actual!r}"
            )
...
def rollout_learning_rate(config: OrpheusConfig) -> float:
    ...
    return float(config.training.learning_rate * config.training.closed_loop_learning_rate_scale)
...
def run_oracle_rung(
    ...
    velocity = gradient_batch["objects"]["velocity"][:, 0, :1].detach().clone().requires_grad_(True)
...
def measurement_metrics(
    ...
    relative_radius_error = (estimate.radius_pixels - frame["radius_pixels"]) / frame[
        "radius_pixels"
    ].clamp_min(1.0e-6)
    return {
        ...
        "centre_rmse_pixels": float(
            ((estimate.centres - frame["centre"]) * pixel_scale).square().mean().sqrt()
        ),
        ...
        "minimum_photometric_confidence": float(estimate.photometric_radius.confidence.min()),
        "maximum_photometric_fit_rms": float(estimate.photometric_radius.fit_mse.max().sqrt()),
        ...
        "photometric_finite_fraction": float(
            estimate.photometric_radius.all_stage_finite_mask.to(torch.float32).mean()
        ),
        "photometric_final_fit_valid_fraction": float(
            estimate.photometric_radius.final_fit_valid_mask.to(torch.float32).mean()
        ),
        "photometric_monotonic_fraction": float(
            estimate.photometric_radius.stage_monotonic_mask.to(torch.float32).mean()
        ),
        ...
        "valid_fraction": float(
            (estimate.geometry.valid_mask & estimate.photometric_radius.valid_mask)
            .to(torch.float32)
            .mean()
        ),
    }
