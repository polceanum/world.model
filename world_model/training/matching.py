...
def match_measurements_to_targets(
    ...
    *,
    ...
    proposal_mask: Tensor | None = None,
    ...
    if proposal_mask is not None:
        if proposal_mask.shape != (batch, proposals):
            raise ValueError("proposal_mask must have shape [B,M]")
        if proposal_mask.dtype != torch.bool:
            raise TypeError("proposal_mask must be torch.bool")
    ...
    for batch_index in range(batch):
        ...
        valid_proposals = (
            torch.arange(proposals, device=predicted_values.device)
            if proposal_mask is None
            else torch.nonzero(proposal_mask[batch_index], as_tuple=False).flatten()
        )
        if valid_targets.numel() == 0 or valid_proposals.numel() == 0:
            continue
        prediction = predicted_values[batch_index, valid_proposals, :4].detach().cpu()
        ...
        if existence_logits is not None:
            confidence_cost = (
                -existence_logits[batch_index, valid_proposals].detach().cpu().sigmoid()
            )
            cost = cost + 0.05 * confidence_cost[:, None]
        rows, columns = linear_sum_assignment(np.asarray(cost))
        row_tensor = valid_proposals[
            torch.as_tensor(rows, device=valid_proposals.device, dtype=torch.int64)
        ]
