...
- Advanced the authoritative contract to specification 1.55 while retaining
  simulator protocol `sphere_world_v7`. The previously qualified standalone
  two-second RGB-D rung remains unchanged and qualified.
- Implemented the public one-slot bridge, its frozen configuration, and its
  seed-free qualification protocol; independent source/config/protocol review
  passes. The config SHA-256 is
  `c40b3438c7fd60646d356db3fe54050039912ace288d9db89620b626106993a3`, and
  the canonical current-byte seed-free protocol SHA-256 is
  `e536b0d0b721042bff55501faf3445456219fcc987334b6ec1e892688ea560b2`.
- The bridge ingests one composite batched `rgbd` observation with explicit
  image size and calibration. One direct observable metric-position owner
  feeds a raw sixteen-frame history; uniform differentiable WLS fits velocity
  only, and parameter-free analytic dynamics answer every rollout horizon.
  Per-frame VJPs cover current velocity and every horizon through each of the
  sixteen RGB and depth inputs.
- Parameters, state-dict entries, and optimizer updates remain zero. Complete
  batch-four persistent runtime tensor state occupies `25,364` recursively
  enumerated unique-storage bytes against a `32,768`-byte gate. The final
  current-byte targeted gate passes `419` tests in `61.33 s`; the complete
  repository gate passes `1207` tests with `16` skips in `431.10 s`.
- For an already-active persistent object, a well-formed frame with missing
  depth, nonfinite or otherwise invalid depth in the sampled measurement
  support, or no foreground appends an invalid causal row; the frozen
  sixteenth-frame full-window ablation reports `sample_count: 16` and
  `valid_count: 15`. It emits no valid/admissible temporal fit or direct
  velocity evidence, correction, or birth; a finite diagnostic fit with
  `fit_valid: false` is not admissible evidence. Before birth, the same frame
  advances runtime time but creates no
  object-history row and no birth. Malformed packets, nonfinite RGB/
  calibration, low precision, unknown/duplicate/stale streams, and invalid
  prepared propagation reject atomically without consuming runtime or one-use
  propagation state.
- Evaluator/demo paths carry truthful RGB-D modality metadata. The evaluator
  labels its fifteen-frame warmup; demo aggregate errors remain pooled across
  warmup and post-warmup frames. The legacy RGB path is preserved, and exact
  live-stream resume from a mid-history checkpoint is explicitly unsupported.
- Reserved development `45000000--45000023`, selector
  `46000000--46000015`, confirmation `47000000--47000015`, and exactly-once
  final `48000000--48000031` manifests remain unopened. These source and gate
  results are not episode-accuracy evidence. Next is a clean commit/push, one
  development reproduction plus independent audit, then selector ->
  confirmation -> final exactly once. No capacity or task scaling is permitted
  before that bridge qualification passes.
...
- Ran the first development manifest `41000000--41000023` on clean commit
  `8e68035`. All `82` reported scalar metrics were finite and all frozen gates
  passed. Current position/velocity
  RMSE was `0.00279934 m`/`0.00207092 m/s`; the five horizon position RMSEs
  were `0.00286018/0.00297530/0.00322108/0.00385564/0.00539692 m`; perception
  and rollout time was `0.235061 s`/`0.00391524 s`.
- Recorded report SHA-256
  ...
  manifest SHA-256
  `b92f1e3f12475986ebd2971ad2de70187432c0941caa0335ea53e82abc3c1d01`.
- Audit found one source-integrity defect: a raw Python tuple/list equality
  check was representation-sensitive after JSON roundtrip. Replaced it with a
  canonical JSON SHA comparison and added regression coverage.
- The repair changes source, so the otherwise passing first report/checkpoint
  pair is historical conditional evidence and must not qualify protected
  access. A clean-source development rerun to fresh v2 artifact paths is
  pending.
- The repaired source passes `104` focused tests and the complete gate at
  `1130 passed, 16 skipped in 414.82 s`. Selector, confirmation, and final
  42m/43m/44m manifests remain unopened, no protected ledger exists, and no
  fresh development, protected, temporal-convergence, or long-horizon-
  convergence result is claimed.

### 2026-08-26 frozen parameter-free RGB-D temporal protocol

- Advanced the authoritative source contract to specification 1.54 while
  retaining simulator protocol `sphere_world_v7`.
- Froze `configs/rgbd_temporal_free_motion_cpu.yaml` at SHA-256
  `5667cdb3603682b8d80a3e42793d25e36989269df1afacfa9b1028f2451101e9`
  and the canonical `rgbd_temporal_free_motion_v1` payload, before insertion
  of its self-reporting digest field, at SHA-256
  `4e334e9d7942ea3f2416c0a9f5ca8e327d1d0a1e9131074f20c051ebd3163ad7`.
- Declared fresh disjoint development `41000000--41000023`, selector
  `42000000--42000023`, confirmation `43000000--43000023`, and one-shot final
  `44000000--44000047` manifests. At this source-freeze boundary every manifest
  was unopened; no development or protected episode had been generated.
- Added a zero-parameter, zero-buffer, empty-state, zero-optimizer estimator
  that measures all 16 RGB-D frames independently, uses uniform weights in the
  differentiable exact free-motion WLS fit, and queries `AnalyticKinematics`
  at `0.1/0.25/0.5/1.0/2.0 s`.
- Bound current/per-axis/horizon position and velocity, metric measurement,
  trivial-baseline, semigroup, residual/conditioning, memory, and separated
  latency gates. OLS covariance remains explicitly an i.i.d. residual
  diagnostic rather than a calibrated-posterior claim.
- Added fixed-output VJP gates from anchor and every horizon position/velocity
  to both RGB and depth. Added an explicit RGB-only known-radius ablation and
  a fail-closed missing-depth ablation; missing depth never invokes the RGB-only
  path.
- Added exact config/protocol/source/evidence binding, fresh non-aliasing
  artifact paths, externally reviewed report/checkpoint digests, and an
  exclusive durable access ledger that records selector, confirmation, then
  final materialization and stops on first failure.
- The combined seed-free implementation/protocol gate is
  `103 passed`, and independent review passes. At this boundary it was
  source-contract evidence only; no development pass, temporal accuracy,
  protected result, or convergence was claimed.
- The final repository gate passes `1129` tests with `16` expected
  inactive-device skips in `428.18 s`; this is the historical pre-repair gate,
  and no episode manifest was generated by it.
- Recorded the later public bridge boundary: one batched composite `rgbd`
  packet, explicit image size/calibration, modality-qualified sensor state,
  raw metric causal history, active-object invalid rows for well-formed
  missing/no-foreground/invalid-sampled-depth frames, and pre-birth time
  advance without an object row or birth. Neither path falls back to RGB.
...
- This qualified only the single-frame RGB-D metric core. At that boundary a
  new predeclared RGB-D temporal protocol and temporal/long-horizon convergence
  remained pending; the later entry above freezes the protocol only.
