...
- [x] Execute the incumbent across all five new single-factor families and the
  compositional holdout. Camera motion, partial visibility, broad known actions,
  and the compositional physical holdout pass. Sensor noise and variable
  parameters now miss only their uncertainty-coverage boundary.
- [ ] Before any targeted residual attempt, run truth-association,
  truth-parameter, truth-state, and clean-observation planning ablations on the
  remaining sensor/compositional planning failures. Same-seed nominal physical
  restoration is already recorded; boundary-only uncertainty misses do not by
  themselves justify more learned capacity.
- [x] Add and execute factor-conditioned K=8/K=32 planning materializations for
  all six families, with generator-only controls, post-decision private oracles,
  compact reports, and separate dashboard rows. Known actions, physical
  parameters, camera motion, and recovered partial visibility pass. Sensor
  noise and the compositional holdout remain explicit downstream rejections.
- [x] Run the complete pytest/Ruff/format/compile/diff gate after streamed
  factor execution: `2402 passed, 16 skipped`; commit the verified
...
- [x] Preserve the exact qualified source as a verified local Git bundle and
  GitHub archive tag before deleting temporary clones.
- [x] Archive compact run metadata, the historical compatibility fixture, and
  qualified toy evidence; remove the 7.6-GiB stale run/checkpoint tree.
- [x] Remove repository caches, generated demos, disposable temp caches, and
  selected obsolete source clones.
- [x] Remove zero-live-reference rejected profiles and campaign-specific tools
  from the active public surface.
- [x] Preserve superseded status/task/training/accuracy ledgers in Git history
  and the ignored local evidence archive; keep active tracked memory concise.
- [x] Implement and unit-test a differentiable weighted exact free-motion fit
  over a bounded RGB-derived history.
- [x] Freeze the new temporal-rung source, seed namespaces, horizons, gates,
  latency protocol, and artifact schema before selector access.
- [x] Pass focused implementation/static checks without materializing any
  protected episode.
- [x] Pass the pre-failure source gate at `888981861912...`: `1075 passed, 16
  skipped`, Ruff lint and format-check over 224 Python files, compileall,
  specification version, and diff integrity.
- [x] Commit and push the reviewed temporal source as
  `8889818619121351d342490786331e854364532c`, binding development to clean
  exact Git provenance.
- [x] Run the development-only 32-update training and 16-seed audit. Preserve
  its terminal failed report under ignored local archive SHA-256
  `be488d045e259c0804a2a2b24215fa4eb3025d69f6113d8dbefe21d72f827554`.
- [x] Stop monocular temporal architecture attempt 2 of 2 after its 10
  development gate failures; leave selector, confirmation, and final
  namespaces unopened and do not retry the family.
- [x] Remove the rejected temporal config, runner, estimator, and tests from
  the active tree while retaining the reusable exact differentiable
  free-motion primitive and immutable failure evidence.
- [x] Advance the simulator to v7 with observable `[T, 1, H, W]` metric surface
  depth and one exact nearest ray--sphere winner shared by depth, instance,
  visibility, and RGB.
- [x] Add the parameter-free RGB-derived-centre plus bilinear-depth metric
  measurement with perspective radius correction, fail-closed invalid rows,
  ordinary float32/float64 gradients, and explicit low-precision rejection.
- [x] Pass the seed-free 18-case metric grid at `0.00613210 m` maximum and
  `0.00336217 m` RMSE position error, focused `29 passed`, and independent
...
in Git commit `c16acc99` and the local pre-generalization archive.
